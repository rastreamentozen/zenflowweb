  ...
  try {
    ...
    if (resp.getResponseCode() === 200) {
      ...
      if (j && j.length > 0) {
        ...
        // [SÊNIOR FIX]: Fim do histórico fantasma. Lê ESTRITAMENTE o contrato atual.
        let v = arr[0]; 
        ...
        if (comando === "motos") {
          ...
          const cClass = String(v.codigo_classificacao || "");
          const cNome = aba.getRange(linha, MAPA_COLUNAS.NOME + 1);
          
          if (cClass === "1") {
            cNome.setFontColor("#16a34a").setFontWeight("bold").setNote(`✅ Situação SGA: CONCLUÍDO.\nRode "Varrer Concluídos" para removê-lo da fila.\nVerificado: ${dt}`);
            alterado = true;
          } 
          else if (cSit === "1" || cSit === "14" || (cSit === "3" && cClass === "14")) {
            cNome.setFontColor("#000000").setFontWeight("normal").clearNote();
            alterado = true;
          } 
          else {
            cNome.setFontColor("#9C27B0").setFontWeight("bold").setNote(`⚠️ Situação SGA: ${MAPA_SITUACAO_SGA[cSit] || "Desconhecida"}\nClassificação SGA: ${cClass}\nVerificado: ${dt}`);
            alterado = true;
          }
  ...
  todosClientes.forEach((cli, index) => {
    try {
      if (responses[index].getResponseCode() === 200) {
        ...
        if (j && j.length > 0) {
          const arrJ = Array.isArray(j) ? j : [j];
          const v = arrJ[0]; // [SÊNIOR FIX]: Garante remoção segura baseada APENAS no status do dia
          
          if (v && String(v.codigo_classificacao) === "1" && String(v.codigo_situacao) !== "6") {
